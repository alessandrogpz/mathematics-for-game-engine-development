...
TEST(MatricesBasics, ScalarMultiplication) {
    ...
    EXPECT_DOUBLE_EQ((prod[0, 0]), 3.5);
    EXPECT_DOUBLE_EQ((prod[1, 1]), 3.5);
    EXPECT_DOUBLE_EQ((prod[0, 1]), 0.0);
}
...
TEST(MatricesBasics, Transpose) {
    ...
    EXPECT_DOUBLE_EQ((mt[0, 1]), 5.0);
    EXPECT_DOUBLE_EQ((mt[1, 0]), 2.0);
}

TEST(MatricesBasics, Trace) {
    matrices::Matrix4x4 m = matrices::Matrix4x4::identity();
    EXPECT_DOUBLE_EQ(matrices::trace(m), 4.0);

    m[0, 0] = 10.0;
    m[3, 3] = -2.5;
    EXPECT_DOUBLE_EQ(matrices::trace(m), 9.5);
}
