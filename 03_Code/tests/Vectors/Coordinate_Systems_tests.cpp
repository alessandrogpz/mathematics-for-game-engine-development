...
TEST(CoordinateSystems, HandednessConversion) {
    ...
    EXPECT_DOUBLE_EQ(lh.x, 1.0);
    EXPECT_DOUBLE_EQ(lh.y, 2.0);
    EXPECT_DOUBLE_EQ(lh.z, -3.0);

    vectors::vector3 rh = vectors::convertHandedness(lh);
    EXPECT_DOUBLE_EQ(rh.z, 3.0);
}
...
TEST(CoordinateSystems, SphericalCardinalDirections) {
    ...
    EXPECT_DOUBLE_EQ(s1.r, 5.0);
    EXPECT_DOUBLE_EQ(s1.theta, 0.0);
    EXPECT_DOUBLE_EQ(s1.phi, 0.0);
    ...
    EXPECT_DOUBLE_EQ(s2.r, 2.0);
    EXPECT_NEAR(s2.theta, std::numbers::pi / 2.0, 1e-9);
    EXPECT_DOUBLE_EQ(s2.phi, 0.0);
    ...
    EXPECT_NEAR(c3.x, 1.0, 1e-9);
    EXPECT_NEAR(c3.y, 0.0, 1e-9);
    EXPECT_NEAR(c3.z, 0.0, 1e-9);
}
...
TEST(CoordinateSystems, SphericalRoundTrip) {
    ...
    EXPECT_NEAR(round_trip.x, p.x, 1e-9);
    EXPECT_NEAR(round_trip.y, p.y, 1e-9);
    EXPECT_NEAR(round_trip.z, p.z, 1e-9);
}
...
TEST(CoordinateSystems, CylindricalRoundTrip) {
    ...
    // rho = sqrt(x^2 + y^2)
    EXPECT_NEAR(c.rho, std::sqrt(2.5*2.5 + 6.1*6.1), 1e-9);
